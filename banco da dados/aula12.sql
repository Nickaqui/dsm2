...
CREATE OR REPLACE FUNCTION fc_soma(a INTEGER, b INTEGER)
RETURNS INTEGER AS
$$
BEGIN
    RETURN a + b;
...
DECLARE
    ...
    nome_atual TEXT;
BEGIN
    FOR nome_atual, salario_atual IN
        SELECT nome, salario
        FROM tbl_empregados
    LOOP
        IF salario_atual > maior_salario THEN
            maior_salario := salario_atual;
            nome_empregado := nome_atual;
        END IF;
    END LOOP;
...
RETURNS REAL AS $$
DECLARE
    soma_salarios REAL := 0;
    total_empregados INT := 0;
    salario_atual REAL;
BEGIN

    FOR salario_atual IN
        SELECT salario
        FROM tbl_empregados
    LOOP
        soma_salarios := soma_salarios + salario_atual;
    ...
    RETURN soma_salarios / total_empregados;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION fc_salarios(cod_empregado_param INT)
RETURNS TABLE(
    salario_original REAL,
    salario_acrescido_10 REAL,
    salario_reduzido_15 REAL
) AS $$
BEGIN
    RETURN QUERY
    SELECT 
        salario AS salario_original,
        (salario * 1.10)::real AS salario_acrescido_10,
        (salario * 0.85)::real AS salario_reduzido_15
    FROM tbl_empregados
    WHERE cod_empregado = cod_empregado_param;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION fc_proj_departamento(cod_departamento_param INT)
RETURNS INT AS $$
DECLARE
    qtd_projetos INT;
BEGIN
    ...
    INTO qtd_projetos
    FROM tbl_projetos
    WHERE cod_departamento = cod_departamento_param;
